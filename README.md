...
如果你不想手动输入命令，请直接使用项目根目录的 `双击运行.bat`：

1. 把商品链接放到 `input/links.txt`，一行一个链接。
2. 把店小秘模板放到 `input/template.xlsx`。
3. 双击 `双击运行.bat`。
4. 到 `output/店小秘可上传成品表格.xlsx` 获取成品表格。

双击脚本会自动创建 `input/`、`output/`、`output/images/`、`output/reports/`，自动检查 Python、安装依赖、运行程序，并在完成后打开 `output` 文件夹。若缺少 Python，会用中文提示安装 Python 并勾选 `Add python.exe to PATH`。
...
```txt
output/
├── 店小秘可上传成品表格.xlsx             # 可上传店小秘的 Excel 成品表格
├── images/                               # 处理后的图片文件夹
│   └── <sku>/
│       ├── main.jpg
│       ├── scene_1.jpg
