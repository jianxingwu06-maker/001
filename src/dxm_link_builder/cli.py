...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--excel-filename", default="店小秘可上传成品表格.xlsx", help="输出 Excel 主文件名")
    args = parser.parse_args(argv)
    accounts = [item.strip() for item in args.accounts.split(",") if item.strip()]
    result = LinkToDxmPipeline(args.template, args.output, args.platform, accounts, args.rules, args.use_playwright, args.excel_filename).run(load_links(args.links))
    print("生成完成：")
